...
def get_version():
    """
    複数の方法でバージョンを取得する
    優先順位: 1. Git直接 2. version.py 3. フォールバック
    """
    # 1. Gitタグから直接取得（最優先）
    try:
        import subprocess
        ...
        if result.returncode == 0:
            ...
            if git_version.startswith('v.'):
                clean_ver = git_version[2:]
            elif git_version.startswith('v'):
                clean_ver = git_version[1:]
            else:
                clean_ver = git_version
            
            # リリース版以外は -dev をつける
            import re
            if not re.match(r'^\d+\.\d+\.\d+$', clean_ver):
                base_version = re.sub(r'-.*$', '', clean_ver)
                clean_ver = f"{base_version}-dev"
            
            return clean_ver
    except (subprocess.SubprocessError, FileNotFoundError):
        pass
    
    # 2. version.pyから取得（ビルド時生成）
    try:
...
class GitHubUpdater:
    ...
    def create_gui(self):
        """アップデート用GUIの作成"""
        self.root = tk.Tk()
        # self.icon = tk.PhotoImage('assets/icon.ico')
...
def main():
    ...
    updater = GitHubUpdater(
        ...
        github_repo='sdvx_realtime_battle',
